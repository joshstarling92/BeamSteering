function output = Mod_Acquisition(incoming_signal,SV_input_array,sample_frequency,integration_period,intermediate_frequency,NUM_MS,NUM_DATA_SETS)
...
for SV_array = SV_input_array
	...
	for dataset=1:NUM_DATA_SETS;
	    ...
	    for i=1:41
	      ...
	      I = real(incoming_signal(i+value,:));
	      Q = imag(incoming_signal(i+value,:));
	      IQfreq = fft(I+j*Q);
	      codefreq = conj(fft(code));  
